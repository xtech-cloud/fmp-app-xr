...
namespace XTC.FMP.APP.XR
{
    public class XReticleTimer
    {
        ...
        private readonly float DEFAULT_DURATION = 1.5f;
        private float timeOnEnter = 0;
        private float duration;
        private GameObject raycastResultObj_;
        private Renderer rendererComponent_;
        private MeshRenderer meshRenderer_;
        private Image imageGaze_;
        ...
        private void onPointerHoverCallback(XBasePointer _pointer, RaycastResult _raycastResult)
        {
            ...
            if (Time.time - timeOnEnter < duration)
            {
                manger = (360.0f / duration) * (Time.time - timeOnEnter);
            }
            else
            {
                manger = 360f;
            ...
            if (null != imageGaze_)
            {
                imageGaze_.fillAmount = manger / 360f;
                return;
            }
        ...
        private void onPointerEnterCallback(XBasePointer _pointer, RaycastResult _raycastResult)
        {
            ...
            GameObject obj = _raycastResult.gameObject;
            if (null != obj)
            {
                raycastResultObj_ = obj;
                duration = DEFAULT_DURATION;
                Transform imgGaze = obj.transform.Find("__gaze__");
                if (null != imgGaze)
                    imageGaze_ = imgGaze.GetComponent<Image>();
            }
        }
